...
shadowResolution{shadowsResolution}{    
    ...
    #ifndef ios_build
        pointLightMaps = std::unique_ptr<CubemapArray>( new CubemapArray(tf::depth_32, tt::FLOAT, math::Size<unsigned int>{shadowResolution, shadowResolution}, 20) );
...
#endif
                pointLightFramebuffer->clear_frame();
                auto transform = math::transform::translate(math::vec3{0.0f, 0.0f, TRANSFORM});
                switch (j) {
                    case 0:
                        pointLightShadowRenderer->setup_uniforms(pointLight.get_projection(), transform * math::transform::rotate_about_y_radians(static_cast<float>(M_PI/2.0)) * pointLight.get_transformation() );
                        break;
                    case 1:
                        pointLightShadowRenderer->setup_uniforms(pointLight.get_projection(), transform * math::transform::rotate_about_y_radians(-static_cast<float>(M_PI/2.0f)) * pointLight.get_transformation());
                        break;
                    case 2:
                        pointLightShadowRenderer->setup_uniforms(pointLight.get_projection(), transform * math::transform::rotate_about_x_radians(-static_cast<float>(M_PI/2)) * pointLight.get_transformation());
                        break;
                    case 3:
                        pointLightShadowRenderer->setup_uniforms(pointLight.get_projection(), transform * math::transform::rotate_about_x_radians(static_cast<float>(M_PI/2)) * pointLight.get_transformation());
                        break;
                    case 4:
                        ...
                        break;
                    case 5:
                        pointLightShadowRenderer->setup_uniforms(pointLight.get_projection(), transform * math::transform::rotate_about_y_radians(static_cast<float>(M_PI)) * pointLight.get_transformation());
                        break;
                    default:
                        break;
                }
...
void ms::NGin::draw_pl_pov (std::uint16_t x, std::uint16_t y, std::uint16_t tileWidth, std::uint16_t tileHeight) {
    ...
    for(int i = 0; i < scene.get_point_lights().size(); ++i) {
        ...
        scaleRenderer->get_framebuffer().clear_depth();
        pointLightShadowRenderer->setup_uniforms(scene.get_point_lights()[i].get_projection(),transform * math::transform::rotate_about_y_radians(static_cast<float>(M_PI/2)) * scene.get_point_lights()[i].get_transformation());
        for(auto & node : scene.get_nodes()) {
            pointLightShadowRenderer->draw(*node);
        }
        ...
        scaleRenderer->get_framebuffer().clear_depth();
        pointLightShadowRenderer->setup_uniforms(scene.get_point_lights()[i].get_projection(), transform * math::transform::rotate_about_x_radians(-static_cast<float>(M_PI/2)) * scene.get_point_lights()[i].get_transformation());
        for(auto & node : scene.get_nodes()) {
            pointLightShadowRenderer->draw(*node);
        }
    }
