...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods){
    ...
    switch(key) {
        ...
        case GLFW_KEY_J: {
            ...
            engine->scene.get_point_lights()[usedPointLightIndex]->get_positionedObject() = engine->scene.get_point_lights()[usedPointLightIndex]->modelTransformation;
        }
            break;
        case GLFW_KEY_L: {
            auto right = -1.6f * ms::math::right(engine->scene.get_camera().get_transformation());
        ...
        case GLFW_KEY_F: {
            ...
            engine->scene.get_spot_lights()[usedPointLightIndex]->get_positionedObject() = engine->scene.get_spot_lights()[usedSpotLightIndex]->modelTransformation;
        }
            break;
        case GLFW_KEY_H: {
            auto right = -1.6f * ms::math::right(engine->scene.get_camera().get_transformation());
        ...
        case GLFW_KEY_Y: {
            ...
            auto transform = translate<float, 4>(forward);
            engine->scene.get_spot_lights()[usedSpotLightIndex]->modelTransformation.post_transform(transform);
            engine->scene.get_spot_lights()[usedPointLightIndex]->get_positionedObject().post_transform(transform);
        }
            break;
        case GLFW_KEY_8:
